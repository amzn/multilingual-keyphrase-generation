...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        ...
        if self.scheduler_state:
            # : ideally we'd want to just call
        ...
        if cfg.local_rank in [-1, 0]:
            ...
            saving_result_path = os.path.join(cfg.output_dir, "best_ckpt.txt")
            with open(saving_result_path, 'w') as f:
                f.write(str(self.best_cp_name))
        ...
        for i, samples_batch in enumerate(
            ...
            if epoch == 0 and i == 0:
                ...
                self.validate_and_save(
                    -1, train_data_iterator.get_iteration(), scheduler
                )
                self.biencoder.train()
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        if self.cfg.ignore_checkpoint_offset:
            ...
            # : offset doesn't work for multiset currently
...
def _do_biencoder_fwd_pass(
    ...
    if model.training:
        model_out = model(
            ...
            encoder_type=encoder_type,
            representation_token_pos=rep_positions,
            average_token_representation=cfg.encoder.average_token_representation,
        )
    else:
        with torch.no_grad():
            model_out = model(
...
@hydra.main(config_path="conf", config_name="kp_biencoder_train_cfg")
