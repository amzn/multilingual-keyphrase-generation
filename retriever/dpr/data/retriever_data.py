...
class JsonQASrc(QASrc):
    ...
    def load_data(self):
        super().load_data()
        data = []
        jsonl_reader = read_data_from_json_files(self.data_files)
        for jline in jsonl_reader:
            question = jline[self.question_attr]
            answer = [jline[self.answers_attr]] if self.answers_attr in jline else []
            id = None
            if self.id_attr in jline:
                id = jline[self.id_attr]
            data.append(QASample(question, id, answer))
        self.data = data
...
class CsvKPCtxSrc(RetrieverData):
    def __init__(
        self,
        file: str,
        id_col: int = 0,
