...
def get_bert_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    ...
    biencoder = BiEncoder(
        ctx_encoder if cfg.encoder.share_encoder else question_encoder,
        ctx_encoder,
...
def _add_special_tokens(tokenizer, special_tokens):
    ...
    # : this is a hack-y logic that uses some private tokenizer structure which can be changed in HF code
...
class HFBertEncoder(BertModel):
    ...
    def forward(
        ...
        average_token_representation=False,
    ) -> Tuple[T, ...]:
        ...
        if average_token_representation:
            pooled_output = torch.sum(sequence_output[:,1:-1,:] * attention_mask[:,1:-1].unsqueeze(-1), 1) / torch.sum(attention_mask[:,1:-1], 1, keepdim=True)
        else:
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
        ...
        # : move max len to methods params?
