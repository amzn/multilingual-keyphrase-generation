...
from dpr.data.qa_validation import QAMatchStats
...
def validate_kp(
    ...
    scores = []
    for qa, top_k in zip(qa_src.data, result_ctx_ids):
        if len(qa.answers) == 0: continue
        if '_' in qa.id:
            qa_id = qa.id.split('_')[1]
        else:
            qa_id = qa.id
        scores.append([x == qa_id for x in top_k[0]])

    n_docs = len(result_ctx_ids[0][0])
    top_k_hits = [0] * n_docs
    ...
    for k in [1,2,3,4,5,10,20,50,100]:
        print(f'Top {k}: {top_k_hits[k-1]}')
    return {k: top_k_hits[k-1] for k in [1,2,3,4,5,10,20,50,100]}
    # return match_stats.questions_doc_hits
...
@hydra.main(config_path="conf", config_name="kp_dense_retriever")
def main(cfg: DictConfig):
    ...
    for ds_item in qa_src.data:
        question, answers, id = ds_item.query, ds_item.answers, ds_item.id
    ...
    validate_kp_top_k_recall = validate_kp(qa_src, top_ids_and_scores)

    if cfg.out_file:
        output = {}
        assert len(asin_ids) == len(top_ids_and_scores)
        for i in range(len(top_ids_and_scores)):
            top_ids_and_scores[i] = list(top_ids_and_scores[i])
            top_ids_and_scores[i][1] = top_ids_and_scores[i][1].tolist()
            output[asin_ids[i]] = top_ids_and_scores[i]
        assert len(top_ids_and_scores) == len(output) == len(asin_ids)
        with open(cfg.out_file, "w") as writer:
            json.dump(output, writer)
        with open(cfg.out_file+'_result.json', 'w') as f:
            json.dump(validate_kp_top_k_recall, f)
        print(f"Saving out_file to {cfg.out_file}")
        print(f"Saving out_file_result to {cfg.out_file}_result.json")
        # save_results(
        #     all_passages,
        #     questions,
        #     question_answers,
